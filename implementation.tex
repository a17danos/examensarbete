...
%can I use hashes in parentheses instead of footnotes when refering to Github?
...
\begin{comment}
Late in the project the layout and the runtime was combined to create the final (technical) artefact
It was executed on an imac 27” late 2015
The numbers was copy pasted into a spreadsheet
Where the best, worst, average, and standard deviation were control calculated (show decscriptive statics)
The data is shown in figure x
The data was also used to calculate inferential statistics in the form of a 1/2-way anova test (alpha=0.001). The test shows that for the use case of x, there is a significant difference/ that webassembly is significantly more performant than asm.js

In the next chapter more use cases will be added, the benchmarking will be executed on in multiple web browsers on a number of computers and mobile devices to know if the result of the pilot study holds true in other scenarios such as part of a mobile web app.
\end{comment}

%\subsubsection{Gathering results}

%\subsubsection{Crunching the numbers}
